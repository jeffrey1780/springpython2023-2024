...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    for i in range(3):
        for j in range(3):
            find_possible_values = grid[block_row * 3 + 1][block_col * 3 + j]
            find_possible_values.append(find_possible_values)

            return find_possible_values
...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    True
    """
    get_row = set(grid[pos[0]])
    get_col = set(grid[i][pos[1]]for i in range(9))
    get_block = set(
        grid[i][j]
    ...
    return all_values - get_row - get_block - get_col
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    get_row,get_col = empty_position

    #Find all possible values for the empty position
    possible_values = find_possible_values(grid, empty_position)

    #try each possible values
    for value in possible_values:
        grid[get_row][get_col] = value

        #Recursively solve the remaining puzzle
        if solve(grid) is not None:
            return grid

        #If the current value lead to an invalid solution, bacaktrack
        grid[get_row][get_col] = '.'

        #If no solutions is found, return None
...
def generate_sudoku(N: int) -> tp.List[tp.List[str]]:
    ...
    True
    """
    grid = solve([['.' for i in range(9)] for j in range(90)])
    N = 81 - N
